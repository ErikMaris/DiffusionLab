...
%         function out = get.timeLapseMode(obj)
%             if isscalar(obj.dt) == 1 && isnumeric(obj.dt) &&...
%                     isscalar(obj.dte) == 1 && isnumeric(obj.dte)
%                 out = true;
%             else
%                 out = false;
%             end
...
%         function obj = set.dtSDidx(obj,value)
...
%         end
    ...
    methods (Static,Hidden)
        ...
        function Y = roundn(X,N)
            ...
            % Computes the motion blur coefficient from the frame time
            % (obj.dt) and exposure time (obj.dte) based on Eq. S9 in 
            % M. Lind?n and J. Elf. Biophys. J. 115.2 (2018): 276-282
            % 
            % -------------------------------------------------------------
            % -                         USAGE                             -
            % -------------------------------------------------------------
        ...
        function wm = weightedmean(x, w)
            % ROUNDN Computes the weighted mean
            % 
            % 
            % -------------------------------------------------------------
            % -                         USAGE                             -
        ...
        function sewm = standarderrorweightedmean(x, w)
            % ROUNDN Computes the standard error weighted mean
            % 
            % 
            % -------------------------------------------------------------
            % -                         USAGE                             -
    ...
    methods (Static)
        ...
        outStruct = funFinder_SDCPDfit(nMobile)
        ...
        obj = importTrackStruct(filepath,structName,pixel_m,time_s)
        obj = importCOMSOL(csvpath,nDim,pixel_m,time_s)
        varargout = simBDvec(nDim,nParticles,nTimeSteps,dT,D,varargin)
