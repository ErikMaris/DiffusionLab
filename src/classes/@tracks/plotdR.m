function varargout = plotdR(obj, ha, indices, delayIdx, logbinFlag, normalization, Xedges)
...
% -------------------------------------------------------------
% 
% plotdR(obj, ha, indices, delayIdx, logbinFlag, normalization)
...
if nargin < 5 || isempty(logbinFlag)
    logbinFlag = false;
end
...
if nObj > 1 % plot multiple populations; get cell with this
    ...
    for ii = 1:nObj
        ...
        [uF,uL] = obj(ii).getUnitFactor('pixelsize');
        data{ii} = sqrt(data{ii}).*uF;
    end
    nPlot = numel(obj);
    ...
    [uF,uL] = obj.getUnitFactor('pixelsize');
    data = cellfun(@(x) sqrt(x).*uF, data,'UniformOutput',false);
